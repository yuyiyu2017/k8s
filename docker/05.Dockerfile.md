...
![Alt text](./picture/05.01.volume实质.png)
...
> 自定义配置文件，并加载初始sql
> 
> 只要放在 /docker-entrypoint-initdb.d/ 目录下的sql，创建容器时会自动加载
```
FROM mysql:5.7
...
## 6、Fastdfs 使用示例
> 参照 https://github.com/yuyiyu2017/fastdfs
>
> 自定义配置文件和启动脚本
```
FROM yuyiyu/fastdfs:2.0
COPY ./conf /etc/fdfs/
